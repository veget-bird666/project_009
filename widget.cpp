...
#include <QDebug>
#include <cmath> // 引入cmath库以使用 floor 函数
...
void Widget::paintEvent(QPaintEvent *event)
{
    ...
    for(int i=0;i<row;i++){
        for(int j=0;j<col;j++){
            ...
            if(space_type == -1){
                painter.drawPixmap(j_off+j*block_size,i_off+i*block_size,
                                   block_size, block_size, pic_curtain.scaled(block_size, block_size, Qt::KeepAspectRatio));
            }
        }
    }
    ...
    // 绘制选中框
    int m_x = m_mousePos.x();
    int m_y = m_mousePos.y();

    int stay_i =floor( (m_y-i_off)/block_size*1.0);
    int stay_j =floor( (m_x-j_off)/block_size*1.0);

    int stay_x = stay_j*block_size + j_off;
    int stay_y = stay_i*block_size + i_off;

    qDebug()<<j_off<<","<<i_off;
    qDebug()<<m_mousePos.x()<<","<<m_mousePos.y();
    qDebug()<<stay_j<<","<<stay_i;
    qDebug()<<stay_x<<","<<stay_y;

    if(stay_i>=0 && stay_j>=0 && stay_i<row && stay_j<col){
        QColor color(255, 255, 100, 250);
        // 设置画笔的粗细，单位为像素

        painter.setPen(color);
        QPen pen = painter.pen();
        pen.setWidth(4); // 设置粗细为5像素
        painter.setPen(pen);
        painter.drawLine(stay_x,stay_y,stay_x+18,stay_y);
        painter.drawLine(stay_x,stay_y,stay_x,stay_y+18);
        painter.drawLine(stay_x+block_size,stay_y,stay_x+block_size-18,stay_y);
        painter.drawLine(stay_x+block_size,stay_y,stay_x+block_size,stay_y+18);
        painter.drawLine(stay_x,stay_y+block_size,stay_x,stay_y+block_size-18);
        painter.drawLine(stay_x,stay_y+block_size,stay_x+18,stay_y+block_size);
        painter.drawLine(stay_x+block_size,stay_y+block_size,stay_x+block_size-18,stay_y+block_size);
        painter.drawLine(stay_x+block_size,stay_y+block_size,stay_x+block_size,stay_y+block_size-18);
...
void Widget::picInitial(){
    ...
    pic_curtain.load(":/material/picture/mine/curtain.png");
...
void mapInitial() {
    ...
    spaces[3][1]->SetType(3);
    spaces[4][1]->SetType(3);
    ...
    spaces[0][0]->SetType(-1);
    spaces[1][1]->SetType(-1);
    spaces[2][2]->SetType(-1);
